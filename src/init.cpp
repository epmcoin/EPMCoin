...
#include "config/extreme_private_masternode-config.h"
#endif
...
void PrepareShutdown()
{
	...
	RenameThread("extreme_private_masternode-shutoff");
...
void Shutdown()
{
	...
	if (nLocalServices == NODE_BLOOM_LIGHT_ZC) {
		lightWorker.StopLightZepmThread();
	}
#ifdef ENABLE_WALLET
	delete pwalletMain;
	pwalletMain = NULL;
...
std::string HelpMessage(HelpMessageMode mode)
{
	...
	strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "extreme_private_masternode.conf"));
...
#ifndef WIN32
	strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "extreme_private_masternoded.pid"));
#endif
	strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
	strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
	strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the EPM and zEPM money supply statistics") + " " + _("on startup"));
	...
	strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 2680, 51474));
	...
	if (GetBoolArg("-help-debug", false))
		strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in EPM/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
			FormatMoney(CWallet::minTxFee.GetFeePerK())));
	strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in EPM/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
	...
	string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, extreme_private_masternode, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero, precompute, staking)"; // Don't translate these and qt below
	if (mode == HMM_BITCOIN_QT)
		debugCategories += ", qt";
	...
	strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in EPM/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
	...
	strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all Extreme Private Masternode specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));

#ifdef ENABLE_WALLET
	strUsage += HelpMessageGroup(_("Staking options:"));
	strUsage += HelpMessageOpt("-staking=<n>", strprintf(_("Enable staking functionality (0-1, default: %u)"), 1));
	strUsage += HelpMessageOpt("-epmstake=<n>", strprintf(_("Enable or disable staking functionality for EPM inputs (0-1, default: %u)"), 1));
	strUsage += HelpMessageOpt("-zepmstake=<n>", strprintf(_("Enable or disable staking functionality for zEPM inputs (0-1, default: %u)"), 1));
	...
	strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:2680"));
	...
	strUsage += HelpMessageOpt("-backupzepm=<n>", strprintf(_("Enable automatic wallet backups triggered after each zEPM minting (0-1, default: %u)"), 1));
	strUsage += HelpMessageOpt("-precompute=<n>", strprintf(_("Enable precomputation of zEPM spends and stakes (0-1, default %u)"), 1));
	...
	strUsage += HelpMessageOpt("-zepmbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zEPM backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
	...
	strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 2679, 51475));
...
std::string LicenseInfo()
{
	return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
		...
		"\n" +
		FormatParagraph(strprintf(_("Copyright (C) 2015-%i The Extreme Private Masternode Coin Developers"), COPYRIGHT_YEAR)) + "\n" +
		"\n" +
...
{
	RenameThread("extreme_private_masternode-loadblk");
...
/** Sanity checks
 *  Ensure that Extreme Private Masternode is running in a usable environment with all
...
/** Initialize extreme_private_masternode.
...
#define PROCESS_DEP_ENABLE 0x00000001
	...
	// Sanity check
	if (!InitSanityCheck())
		return InitError(_("Initialization sanity check failed. Extreme Private Masternode Coin is shutting down."));
...
#endif
	// Make sure only a single Extreme Private Masternode process is using the data directory.
	...
	if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
		return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Extreme Private Masternode Coin is probably already running."), strDataDir));
	...
	if (GetBoolArg("-shrinkdebugfile", !fDebug))
		ShrinkDebugFile();
	LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
	LogPrintf("EXTREME_PRIVATE_MASTERNODE version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
	...
	while (!fLoaded) {
		...
		do {
			try {
				...
				//EXTREME_PRIVATE_MASTERNODE specific: zerocoin and spork DB's
				...
				// EXTREME_PRIVATE_MASTERNODE: load previous sessions sporks if we have them.
				...
				if (GetBoolArg("-reindexmoneysupply", false)) {
					RecalculateZEPMMinted();
					RecalculateZEPMSpent();
					RecalculateEPMSupply(1);
				}
				...
				if (GetBoolArg("-reindexaccumulators", false)) {
					...
					// EXTREME_PRIVATE_MASTERNODE: recalculate Accumulator Checkpoints that failed to database properly
	...
	if (fDisableWallet) {
		...
		if (nLoadWalletRet != DB_LOAD_OK) {
			if (nLoadWalletRet == DB_CORRUPT)
				...
				strErrors << _("Error loading wallet.dat: Wallet requires newer version of Extreme Private Masternode Coin") << "\n";
			else if (nLoadWalletRet == DB_NEED_REWRITE) {
				strErrors << _("Wallet needed to be rewritten: restart Extreme Private Masternode Coin to complete") << "\n";
		...
		zwalletMain = new CzEPMWallet(pwalletMain->strWalletFile);
		...
		//Inititalize zEPMWallet
		uiInterface.InitMessage(_("Syncing zEPM wallet..."));

		pwalletMain->InitAutoConvertAddresses();

		bool fEnableZEpmBackups = GetBoolArg("-backupzepm", true);
		pwalletMain->setZEpmAutoBackups(fEnableZEpmBackups);

		//Load zerocoin mint hashes to memory
		pwalletMain->zepmTracker->Init();
	...
	/* Denominations
	   ...
	   For example:
	   1EPM+1000 == (.1EPM+100)*10
	   10EPM+10000 == (1EPM+1000)*10
	*/
	...
	if (nLocalServices & NODE_BLOOM_LIGHT_ZC) {
		...
		lightWorker.StartLightZepmThread(threadGroup);
	}
	...
	if (pwalletMain) {
		...
		if (GetBoolArg("-precompute", false)) {
			// Run a thread to precompute any zEPM spends
