...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total extreme_private_masternode balance of the wallet (excluding zerocoins)\n"
            ...
            "  \"zEPMsupply\" :\n"
            "  {\n"
            "     \"1\" : n,            (numeric) supply of 1 zEPM denomination\n"
            "     \"5\" : n,            (numeric) supply of 5 zEPM denomination\n"
            "     \"10\" : n,           (numeric) supply of 10 zEPM denomination\n"
            "     \"50\" : n,           (numeric) supply of 50 zEPM denomination\n"
            "     \"100\" : n,          (numeric) supply of 100 zEPM denomination\n"
            "     \"500\" : n,          (numeric) supply of 500 zEPM denomination\n"
            "     \"1000\" : n,         (numeric) supply of 1000 zEPM denomination\n"
            "     \"5000\" : n,         (numeric) supply of 5000 zEPM denomination\n"
            "     \"total\" : n,        (numeric) The total supply of all zEPM denominations\n"
            "  }\n"
            "  \"keypoololdest\": xxxxxx,    (numeric) the timestamp (seconds since GMT epoch) of the oldest pre-generated key in the key pool\n"
            "  \"keypoolsize\": xxxx,        (numeric) how many new keys are pre-generated\n"
            "  \"unlocked_until\": ttt,      (numeric) the timestamp in seconds since epoch (midnight Jan 1 1970 GMT) that the wallet is unlocked for transfers, or 0 if the wallet is locked\n"
            "  \"paytxfee\": x.xxxx,         (numeric) the transaction fee set in extreme_private_masternode/kb\n"
            "  \"relayfee\": x.xxxx,         (numeric) minimum relay fee for non-free transactions in extreme_private_masternode/kb\n"
    ...
    UniValue zepmObj(UniValue::VOBJ);
    for (auto denom : libzerocoin::zerocoinDenomList) {
        zepmObj.push_back(Pair(to_string(denom), ValueFromAmount(chainActive.Tip()->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    zepmObj.push_back(Pair("total", ValueFromAmount(chainActive.Tip()->GetZerocoinSupply())));
    obj.push_back(Pair("zEPMsupply", zepmObj));
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"extreme_private_masternodeaddress\"\n"
            "\nReturn information about the given extreme_private_masternode address.\n"

            "\nArguments:\n"
            "1. \"extreme_private_masternodeaddress\"     (string, required) The extreme_private_masternode address to validate\n"

            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"extreme_private_masternodeaddress\", (string) The extreme_private_masternode address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: Extreme Private Masternode address and we have full public key:
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
        throw runtime_error(
            ...
            "2. \"keys\"       (string, required) A json array of keys which are extreme_private_masternode addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) extreme_private_masternode address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"extreme_private_masternodeaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"

            "\nArguments:\n"
            "1. \"extreme_private_masternodeaddress\"  (string, required) The extreme_private_masternode address to use for the signature.\n"
