...
/**
 * Extreme Private Masternode RPC command dispatcher.
 */
class CRPCTable
{
private:
...
extern UniValue DoZepmSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, vector<CZerocoinMint>& vMintsSelected, std::string address_str, bool isPublicSpend = true);
...
extern UniValue setzepmseed(const UniValue& params, bool fHelp);
extern UniValue getzepmseed(const UniValue& params, bool fHelp);
extern UniValue generatemintlist(const UniValue& params, bool fHelp);
extern UniValue searchdzepm(const UniValue& params, bool fHelp);
extern UniValue dzepmstate(const UniValue& params, bool fHelp);
