...
public:
    ...
    bool WriteZEPMSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZEPMSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZEPMSeed_deprecated(uint256& seed);
    bool EraseZEPMSeed();
    bool EraseZEPMSeed_deprecated();

    bool WriteZEPMCount(const uint32_t& nCount);
    bool ReadZEPMCount(uint32_t& nCount);
