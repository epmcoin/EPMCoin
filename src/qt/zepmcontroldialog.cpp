...
#include "ui_zepmcontroldialog.h"
...
std::set<std::string> ZEpmControlDialog::setSelectedMints;
std::set<CMintMeta> ZEpmControlDialog::setMints;

bool CZEpmControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZEpmControlDialog::COLUMN_DENOMINATION || column == ZEpmControlDialog::COLUMN_VERSION || column == ZEpmControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZEpmControlDialog::ZEpmControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZEpmControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZEpmControlDialog::~ZEpmControlDialog()
{
    delete ui;
}

void ZEpmControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZEpmControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZEpmControlWidgetItem* itemDenom(new CZEpmControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZEpmControlWidgetItem *itemMint = new CZEpmControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        {
            LOCK(pwalletMain->zepmTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zepmTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zEPM.";
            else if (!mint.isSeedCorrect)
                strReason = "The zEPM seed used to mint this zEPM is not the same as currently hold in the wallet";
            else
...
void ZEpmControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZEpmControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZEpm_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZEpmControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZEpmControlDialog::GetSelectedMints()
{
...
void ZEpmControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
