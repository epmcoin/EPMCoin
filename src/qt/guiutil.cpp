...
#define URI_SCHEME "extreme_private_masternode"
...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Extreme Private Masternode address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
...
{
    // return if URI is not valid or is no EXTREME_PRIVATE_MASTERNODE: URI
...
{
    // Convert extreme_private_masternode:// to extreme_private_masternode:
    //
    //    Cannot handle this later, because extreme_private_masternode:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open extreme_private_masternode.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "EXTREME_PRIVATE_MASTERNODE.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for EXTREME_PRIVATE_MASTERNODE.lnk
...
{
    return GetAutostartDir() / "extreme_private_masternode.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a extreme_private_masternode.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=EXTREME_PRIVATE_MASTERNODE\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the extreme_private_masternode app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add extreme_private_masternode app to startup item list
