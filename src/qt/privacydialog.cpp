                                                          ...
                                                          fDenomsMinimized(true)
{
    ...
    // "Spending 999999 zEPM ought to be enough for anybody." - Bill Gates, 2017
    ui->zEPMpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzEPMSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    */
    ...
    // Extreme Private Masternode settings
    QSettings settings;
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zEPMpayAmount->setFocus();
    }
}
/* disable MINT
 *
void PrivacyDialog::on_pushButtonMintzEPM_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zEPM is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Mint_zEPM, true));
        if (!ctx.isValid()) {
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zEPM...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zEPM in ") +
...
void PrivacyDialog::on_pushButtonSpendzEPM_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zEPM is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Send_zEPM, true));
        if (!ctx.isValid()) {
            // Unlock wallet was cancelled
            return;
        }
        // Wallet is unlocked now, sedn zEPM
        sendzEPM();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zEPM
    sendzEPM();
}

void PrivacyDialog::on_pushButtonZEpmControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;

    ZEpmControlDialog* zEpmControl = new ZEpmControlDialog(this);
    zEpmControl->setModel(walletModel);
    zEpmControl->exec();
}

void PrivacyDialog::setZEpmControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzEpmSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzEPM()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Extreme_Private_Masternode Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zEPMpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zEPM is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " EPM </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zEPM</b>";
    ...
    // use mints from zEPM selector if applicable
    vector<CMintMeta> vMintsToFetch;
    vector<CZerocoinMint> vMintsSelected;
    if (!ZEpmControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZEpmControlDialog::GetSelectedMints();
    ...
    if (!fSuccess) {
        ...
        const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zEPM transaction
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zEPM was spent successfully update the addressbook with the label
    ...
    // Clear zepm selector in case it was used
    ZEpmControlDialog::setSelectedMints.clear();
    ui->labelzEpmSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zEPM Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Epm, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.IsZerocoinMint())
            strStats += tr("zEPM Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    std::vector<CMintMeta> vMints = pwalletMain->zepmTracker->GetMints(false);
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zEPM </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zEPM "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zEPM "));
    ui->labelzAvailableAmount_4->setText(QString::number(zerocoinBalance/COIN) + QString(" zEPM "));
    ...
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zEPM </b> "));
    ui->labelZsupplyAmount_2->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zEPM </b> "));
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " zEPM </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
...
{
    ui->labelzEPMSyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    //bool fButtonsEnabled =  ui->pushButtonMintzEPM->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zEPM
        //ui->pushButtonMintzEPM->setEnabled(false);
        //ui->pushButtonMintzEPM->setToolTip(tr("zEPM is currently disabled due to maintenance."));

        // Spend zEPM
        ui->pushButtonSpendzEPM->setEnabled(false);
        ui->pushButtonSpendzEPM->setToolTip(tr("zEPM is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zEPM
        //ui->pushButtonMintzEPM->setEnabled(true);
        //ui->pushButtonMintzEPM->setToolTip(tr("PrivacyDialog", "Enter an amount of EPM to convert to zEPM", 0));

        // Spend zEPM
        ui->pushButtonSpendzEPM->setEnabled(true);
        ui->pushButtonSpendzEPM->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}
