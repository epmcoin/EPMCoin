...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered extreme_private_masternode address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 EPM.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal extreme_private_masternode:URI (extreme_private_masternode:XyZ...?message=example)
            {
...
static void NotifyzEPMReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzEPMReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzEPMReset.connect(boost::bind(NotifyzEPMReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzEPMReset.disconnect(boost::bind(NotifyzEPMReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zepmTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus, fWrongSeed);
}
