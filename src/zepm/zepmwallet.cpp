...
CzEPMWallet::CzEPMWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zepm seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZEPMSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZEPMSeed_deprecated()) {
                    LogPrintf("%s: Updated zEPM seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zepm seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zepm wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzEPMWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZEPMCount(nCountLastUsed);
    else if (!walletdb.ReadZEPMCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzEPMWallet::Lock()
{
    seedMaster = 0;
}

void CzEPMWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzEPMWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZEPM(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzEPMWallet::LoadMintPoolFromDB()
{
...
void CzEPMWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzEPMWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzEPMWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zepmTracker->HasPubcoinHash(pMint.first)) {
...
bool CzEPMWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZEPM(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zepmTracker which also adds to database
    pwalletMain->zepmTracker->Add(dMint, true);
...
void CzEPMWallet::SeedToZEPM(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzEPMWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzEPMWallet::GenerateDeterministicZEPM(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzEPMWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzEPMWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzEPMWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
