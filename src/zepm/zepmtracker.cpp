...
CzEPMTracker::CzEPMTracker(std::string strWalletFile)
{
...
CzEPMTracker::~CzEPMTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzEPMTracker::Init()
{
...
bool CzEPMTracker::Archive(CMintMeta& meta)
{
...
bool CzEPMTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzEPMTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzEPMTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzEPMTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzEPMTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzEPMTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzEPMTracker::GetSerialHashes()
{
...
CAmount CzEPMTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzEPMTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzEPMTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzEPMTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzEPMTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzEPMTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzEPMTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzEPMTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzEPMTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzEPMTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzEPMTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzEPMWallet* zEPMWallet)
{
    bool iszEPMWalletInitialized = (NULL != zEPMWallet);
    CMintMeta meta;
    ...
    if (! iszEPMWalletInitialized)
        zEPMWallet = new CzEPMWallet(strWalletFile);
    meta.isSeedCorrect = zEPMWallet->CheckSeed(dMint);
    if (! iszEPMWalletInitialized)
        delete zEPMWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzEPMTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzEPMTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzEPMTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzEPMTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzEPMTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzEPMTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzEPMWallet* zEPMWallet = new CzEPMWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zEPMWallet);
        }
        delete zEPMWallet;
        LogPrint("zero", "%s: added %d dzepm from DB\n", __func__, listDeterministicDB.size());
    }
