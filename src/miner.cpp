...
//////////////////////////////////////////////////////////////////////////////
//
// EXTREME_PRIVATE_MASTERNODEMiner
//
...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
...

	{
		...
		for (map<uint256, CTxMemPoolEntry>::iterator mi = mempool.mapTx.begin();
			...
			for (const CTxIn& txin : tx.vin) {
				...
				if (hasZerocoinSpends) {
					...
					//Priority = (age^6+100000)*amount - gives higher priority to zepms that have been in mempool long
					//and higher priority to zepms that are large in value
					...
					// zEPM spends can have very large priority, use non-overflowing safe functions
		...
		while (!vecPriority.empty()) {
			...
			// double check that there are no double spent zEPM spends in this block or tx
			if (tx.HasZerocoinSpendInputs()) {
				...
				//This zEPM serial has already been included in the block, do not add this tx.
				if (fDoubleSerial)
					continue;
			}
		...
		if (fProofOfStake) {
			...
			if (pblock->IsZerocoinStake()) {
				...
				if (!pwallet->GetZerocoinKey(bnSerial, key)) {
					LogPrintf("%s: failed to find zEPM with serial %s, unable to sign block\n", __func__, bnSerial.GetHex());
					return NULL;
				}

				//Sign block with the zEPM key
				if (!SignBlockWithKey(*pblock, key)) {
					LogPrintf("BitcoinMiner(): Signing new block with zEPM key failed \n");
					return NULL;
				}
			}
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
	...
	// Found a solution
	{
		...
		if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
			return error("EXTREME_PRIVATE_MASTERNODEMiner : generated block is stale");
	}
	...
	if (!ProcessNewBlock(state, NULL, pblock)) {
		if (pblock->IsZerocoinStake()) {
			pwalletMain->zepmTracker->RemovePending(pblock->vtx[1].GetHash());
			pwalletMain->zepmTracker->ListMints(true, true, true); //update the state
		}
		return error("EXTREME_PRIVATE_MASTERNODEMiner : ProcessNewBlock, block not accepted");
	}
...
{
	LogPrintf("EXTREME_PRIVATE_MASTERNODEMiner started\n");
	SetThreadPriority(THREAD_PRIORITY_LOWEST);
	RenameThread("extreme_private_masternode-miner");
	...
	while (fGenerateBitcoins || fProofOfStake) {
		...
		LogPrintf("Running EXTREME_PRIVATE_MASTERNODEMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
