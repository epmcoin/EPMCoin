...
- [Development guidelines](#development-guidelines)
    - [General Extreme Private Masternode Coin](#general-extreme_private_masternode-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Extreme Private Masternode Coin uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play EPMs (tEPM)" on the test network, if you
...
Extreme Private Masternode Coin is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/extreme_private_masternode-project/extreme_private_masternode/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_extreme_private_masternode
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_extreme_private_masternode --log_level=test_suite
$ valgrind -v --leak-check=full src/extreme_private_masternoded -printtoconsole
```
...
# A coverage report will now be accessible at `./test_extreme_private_masternode.coverage/index.html`.
```
...
- BitcoinMiner : Generates EPMs (if wallet is enabled).
...
However, in open source software such as Extreme Private Masternode Coin, where everyone uses
...
pay attention to for reviewers of Extreme Private Masternode Coin code.

General Extreme Private Masternode Coin
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Extreme Private Masternode Coin uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against Extreme Private Masternode Coin so that they can be integrated
...
file descriptors it will cause problems with EXTREME_PRIVATE_MASTERNODE's `select()` loop, because
...
```bash
$ lsof -p $(pidof extreme_private_masternoded) |\
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:EXTREME_PRIVATE_MASTERNODE-Project/EXTREME_PRIVATE_MASTERNODE.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `extreme_private_masternode-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `extreme_private_masternode-cli` and the GUI debug console use this table to determine how to
