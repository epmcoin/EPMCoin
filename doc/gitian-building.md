...
*Setup instructions for a gitian build of Extreme Private Masternode using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the EXTREME_PRIVATE_MASTERNODE
...
to extreme_private_masternode-crypto.com.
...
- [Building EXTREME_PRIVATE_MASTERNODE](#building-extreme_private_masternode)
...
Clone the git repositories for extreme_private_masternode and gitian and then checkout the extreme_private_masternode version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/extreme_private_masternode-crypto/extreme_private_masternode.git
cd extreme_private_masternode
...
Follow the instructions in [doc/release-process.md](release-process.md) in the extreme_private_masternode repository
under 'Fetch and build inputs' to install sources which require manual intervention. Also follow
the next step: 'Seed the Gitian sources cache', which will fetch all necessary source files allowing
for gitian to work offline.

Building EXTREME_PRIVATE_MASTERNODE
----------------

To build Extreme Private Masternode (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the extreme_private_masternode repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/extreme_private_masternode/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/extreme_private_masternode-crypto/extreme_private_masternode
...
For example:
```bash
URL=https://github.com/crowning-/extreme_private_masternode.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit extreme_private_masternode=${COMMIT} --url extreme_private_masternode=${URL} ../extreme_private_masternode/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit extreme_private_masternode=${COMMIT} --url extreme_private_masternode=${URL} ../extreme_private_masternode/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit extreme_private_masternode=${COMMIT} --url extreme_private_masternode=${URL} ../extreme_private_masternode/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/extreme_private_masternode-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/extreme_private_masternode-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/extreme_private_masternode-build.assert
```
...
[extreme_private_masternode/gitian.sigs](https://github.com/extreme_private_masternode-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
